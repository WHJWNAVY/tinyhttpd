...
void httpd_resp_headers(int client, int status_code, const char *content_type,
                        const char *message) {
...
void httpd_error_bad_request(int client) {
  ...
  p_buf += sprintf(p_buf, "<HTML>\r\n<HEAD>\r\n<TITLE>Bad "
                          "Request</TITLE>\r\n</HEAD>\r\n<BODY>\r\n");
  p_buf += sprintf(p_buf, "<H1>Your browser sent a bad request, such as a POST "
                          "without a Content-Length.</H1>");
  p_buf += sprintf(p_buf, "\r\n</BODY>\r\n</HTML>");
...
void httpd_error_not_found(int client) {
  ...
  p_buf += sprintf(p_buf,
                   "<H1>The server could not fulfill your request because the "
                   "resource specified is unavailable or nonexistent.</H1>");
  p_buf += sprintf(p_buf, "\r\n</BODY>\r\n</HTML>");
...
void httpd_serve_file(int client, const char *filename) {
  ...
  buf[1] = '\0';
  while ((numchars > 0) &&
         strcmp("\n", buf)) { /* read & discard httpd_resp_headers */
...
/**********************************************************************/
void httpd_execute_cgi(int client, const char *path, const char *method,
...
void httpd_accept_request(int client) {
  ...
  if (stat(path, &st) == -1) {
    ...
    if (!cgi) {
      ...
      HTTPD_DEBUG("httpd_execute_cgi [%s] [%s] [%s]", path, method,
                  query_string);
...
#ifdef SERVER_MUTI_THREAD
    if (pthread_create(&newthread, NULL, httpd_accept_request, client_sock) !=
        0) {
